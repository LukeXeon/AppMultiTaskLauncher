...
class TaskInfoProcessor : AbstractProcessor() {
    ...
    private fun TypeSpec.Builder.addGenericAnnotations(): TypeSpec.Builder {
        return addAnnotation(Keep::class)
            .addAnnotation(
    ...
    private fun generateTaskFiles(): List<String> {
    ...
    private fun generateHandlerFiles(): List<String> {
        val filer = processingEnv.filer
        val elementUtils = processingEnv.elementUtils
        val newServices = ArrayList<String>(handlers.size)
        val superclass = handlerInfoType.asClassName()
        for (handler in handlers) {
            val packageName = elementUtils.getPackageOf(handler.type).qualifiedName
            val name = "${handler.type.simpleName}_Runtime_HandlerInfo"
            if (packageName.isNullOrEmpty()) {
                newServices.add(name)
            } else {
                newServices.add("${packageName}.${name}")
    ...
    private fun generateServicesFiles(typeName: String, newServices: List<String>) {
        val filer = processingEnv.filer
        val resourceFile = "META-INF/services/$typeName"
        try {
    ...
    private fun generateFiles() {
        ...
        generateServicesFiles(ANDROID_TASK_INFO_CLASS_NAME, tasks)
        generateServicesFiles(ANDROID_HANDLER_INFO_CLASS_NAME, handlers)
    }
